...
//     const [upcomingTrips, tripHistory] = await Promise.all([
//       Booking.find({ email: user.email, requestStatus: { $in: ['APPROVED', 'PENDING'] } }),
//       Booking.find({ email: user.email, requestStatus: 'COMPLETED' })
//     ]);
...
// exports.updateProfile = async (req, res) => {
//   try {
//     const { firstName, lastName, phone, location, profileUrl } = req.body;
...
//     const bookings = await Booking.find({ email: req.user.email })
//       .sort({ createdAt: -1 })
//       .populate('tripId', 'title image duration');

//     res.json(bookings.map(booking => ({
//       id: booking._id,
//       title: booking.tripId?.title || 'Deleted Trip',
//       image: booking.tripId?.image || '',
//       duration: booking.tripId?.duration || '',
//       dates: `${booking.startDate} - ${booking.endDate}`,
//       status: booking.requestStatus,
//       location: booking.current_location,
//       price: `₹${booking.payment.grandTotal}`,
//       bookingId: `#${booking._id.toString().slice(-6).toUpperCase()}`,
//       totalPaid: `₹${booking.payment.grandTotal}`,
//       highlights: booking.tripId?.activities?.slice(0, 3) || []
//     })));
//   } catch (error) {
//     res.status(500).json({ error: error.message });
//   }
...
//         changes: req.body.reason || 'Cancelled by user' 
//       },
//       { new: true }
//     );
...
//       location: trip.state,
//       price: `₹${trip.payment.grandTotal}`,
//       highlights: trip.activities,
//       includes: trip.inclusions,
//       itinerary: trip.itinerary.reduce((acc, day) => {
//         acc[`Day ${day.dayNumber}`] = day.description;
//         return acc;
//       }, {})
//     };
